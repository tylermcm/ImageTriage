...
# Pinned, hash-verified dependency sets, one per interpreter tag and variant.
# Generated by scripts/refresh_ai_runtime_lock.py.
LOCK_DIRECTORY = "packaging/ai_runtime_locks"
AI_RUNTIME_REQUIRE_LOCK_ENV = "IMAGE_TRIAGE_AI_REQUIRE_LOCK"
...
AI_RUNTIME_ONNX_CPU_REQUIREMENT = "onnxruntime==1.27.0"
...
AI_RUNTIME_ONNX_GPU_REQUIREMENT = "onnxruntime-gpu==1.26.0"
...
# Exact versions, not floors. An open floor lets two machines install different
# packages from one Image Triage build, which is precisely how a demonstration
# machine drifts away from the validated set. Every version below is the one
# present in the runtime these capabilities were proven against; bump them
# deliberately and regenerate the locks with
# scripts/refresh_ai_runtime_lock.py.
...
AI_RUNTIME_DINO_PIP_REQUIREMENTS = (
    ...
    "safetensors==0.8.0",
    "tokenizers==0.22.2",
    # BiRefNet ships its modeling code via trust_remote_code and imports these
    # directly. Without them subject-mask selection fails at model load on
    # every clean install; nothing else in the runtime pulls them in.
    "einops==0.8.2",
    "kornia==0.8.3",
)
...
AI_RUNTIME_DINO_REQUIRED_MODULE_NAMES = (
    ...
    "safetensors",
    "tokenizers",
    # Imported by BiRefNet's trust_remote_code modeling file.
    "einops",
    "kornia",
)
...
def default_ai_runtime_install_root() -> Path:
    """The managed runtime root for this interpreter.

    Derived from the one canonical managed root (``image_triage.ai_paths``) so
    the runtime, the models and the derived-data caches can never end up on
    different volumes or inside Store Python's virtualized package cache.
    """
    root = managed_runtimes_root() / _python_runtime_tag()
...
def build_ai_runtime_pip_install_args(
    *,
    ...
    if lock is not None:
        ...
        # packages. --require-hashes makes a mismatch a hard failure.
        # The lock already contains the full transitive closure, so pip must
        # not resolve anything of its own: that is what let a GPU profile
        # acquire the CPU onnxruntime alongside onnxruntime-gpu.
        args.extend(["--no-deps", "--require-hashes", "--requirement", str(lock)])
        return args
    release_requires_lock = bool(getattr(sys, "frozen", False) and os.name == "nt")
    if release_requires_lock or (
        os.environ.get(AI_RUNTIME_REQUIRE_LOCK_ENV, "") or ""
    ).strip() == "1":
        raise RuntimeError(
            "No pinned AI runtime lock exists for "
...
def _ai_runtime_pip_requirements_for_variant(variant: str, *, include_dino: bool = True) -> tuple[str, ...]:
    ...
    if normalized == AI_RUNTIME_GPU_VARIANT:
        requirements = tuple(
            ...
            if _distribution_name(requirement) == "onnxruntime"
            else requirement
    ...
    pinned_requirements = tuple(
        requirement
        for requirement in requirements
        if _distribution_name(requirement) not in {"torch", "torchvision"}
    )
    if normalized == AI_RUNTIME_GPU_VARIANT:
...
def _distribution_name(requirement: str) -> str:
    """The distribution a requirement string refers to, ignoring its specifier."""
    for separator in ("==", ">=", "<=", "~=", "!=", ">", "<"):
        head = requirement.partition(separator)[0]
        if head != requirement:
...
def _validate_onnxruntime_exclusivity(site_packages_dir: Path) -> None:
    """Reject a profile holding both ONNX Runtime distributions.

    ``onnxruntime`` and ``onnxruntime-gpu`` install into the same ``onnxruntime``
    package directory. With both present, whichever wrote last decides which
    providers exist, so a GPU profile can silently end up CPU-only.
    """
    cpu = _installed_distribution_version(site_packages_dir, "onnxruntime")
    gpu = _installed_distribution_version(site_packages_dir, "onnxruntime-gpu")
    if cpu and gpu:
        raise RuntimeError(
            "AI runtime validation found both onnxruntime "
            f"({cpu}) and onnxruntime-gpu ({gpu}) installed into one profile. "
            "The GPU providers would be unreliable. The existing runtime was not changed."
        )


def _validate_opencv_exclusivity(site_packages_dir: Path) -> None:
    """Reject the two OpenCV wheels sharing the same ``cv2`` package tree."""
    gui = _installed_distribution_version(site_packages_dir, "opencv-python")
    headless = _installed_distribution_version(site_packages_dir, "opencv-python-headless")
    if gui and headless:
        raise RuntimeError(
            "AI runtime validation found both opencv-python "
            f"({gui}) and opencv-python-headless ({headless}) installed into one profile. "
            "They overwrite the same cv2 package files. The existing runtime was not changed."
        )


def _validate_distribution_records(site_packages_dir: Path) -> None:
    _validate_onnxruntime_exclusivity(site_packages_dir)
    _validate_opencv_exclusivity(site_packages_dir)
    missing: list[str] = []
